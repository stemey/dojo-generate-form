define([
	...
	var WidgetList = declare("dojox.mvc.WidgetList", [_WidgetBase, _Container], {
				...
				if(lang.isArray(value)){
					...
					!this.partialRebuild && lang.isFunction(value.watchElements) && (this._handles = this._handles || []).push(value.watchElements(function(idx, removals, adds){
						if (removals.length==0) {
							_self._addChildren(idx,adds);
						}else	if (adds.length==0) {
							_self._removeChildren(idx,removals);
						}else{
		...
		_removeChildren: function(idx,/*dojox/mvc/StatefulArray*/ removals){
			...
			var self=this;
			for(var i=idx;i<idx+removals.length;i++){ childrenToRemove.push(self.getChildren()[i]); }
			array.forEach(childrenToRemove,function(child){this.removeChild(child);child.destroy();},this);
		},
