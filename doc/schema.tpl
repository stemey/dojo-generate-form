...
<p>
	The following list of attributes was generated by the AttributeFactories, which transform the schema into dijits. These AttributeFactories can provide a json schema describing the attribute they handle. That schema is used for the documentation. Two AttributeFactories are left out. ListAttributefactory handles all attributes with the property array=true and no validTypes property. It delegates to the AttributeFactory that handles the cloned attribute with array=false. ListAttributeFactory does not yet provide any extra properties. The other AttributeFactory missing is the one that handles arrays of complex objects. It has the same properties as `embedded`.
</p>
