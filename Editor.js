	...
	function (array, aspect, lang, declare, Container, at, domConstruct, labelHelper, query, registry) {
		...
		return declare("gform.Editor", [Container], {
			...
			removeChangeIndicators: function () {
				this.modelHandle.removeIndicators();
			},
			postCreate: function () {
			...
			_getChangedCountAttr: function () {
				return this.modelHandle.changedCount;
			},
			...
			bufferChange: function (cb) {
				var me = this;
				var oldBufferOnChange = !!this.bufferOnChange;
				this.bufferOnChange = true;
				try {
					cb();
					me.onChange();
				} finally {
					this.bufferOnChange = oldBufferOnChange;
				}
			},
			onChangeInternally: function () {
				if (!this.bufferOnChange) {
					this.onChange();
				}
			},
